...
static func create(_starting_items: Array[Item] = [], _capacity: int = -1) -> Inventory:
	var i:= Inventory.new()
	i.capacity = _capacity
	i.items = _starting_items
	for _item: Item in i.items:
		_item.inventory_container = i
