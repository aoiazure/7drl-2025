...
func get_action(actor: Actor) -> EAction:
	...
	if Input.is_action_just_pressed(&"move_up"):
		...
		if not is_instance_valid(actor.get_component_or_null(Components.INVENTORY)):
			return null
		
		var inv_controller:= PInventoryController.new()
		inv_controller.prev_controller = self
		actor.set_deferred_thread_group("controller", inv_controller)
